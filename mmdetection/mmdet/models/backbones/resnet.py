...
class SelectFeatures_Conv_3_31_no_SF(nn.Module):
    def __init__(self, inplanes, outplanes, stride=1, ratio=0.5):
        super(SelectFeatures_Conv_3_31_no_SF, self).__init__()
    ...
    def forward(self, x):
        ...
        #channel_importance = self.se_channel(x)

        x_3x3_se = x[:,:int(c*self.ratio),:,:]
        x_1x1_se = x[:,int(c*self.ratio):,:,:]
        out_3x3_gwc = self.gwc(x_3x3_se)
        if self.stride ==2:
            x_3x3_se = self.avgpool_s2_3(x_3x3_se)
        out_3x3_pwc = self.pwc(x_3x3_se)
        out_3x3 = out_3x3_gwc + out_3x3_pwc
        out_3x3 = self.bn1(out_3x3)
        out_3x3_ratio = self.avgpool_add_3(out_3x3).squeeze(dim=2).squeeze(dim=2)

        # use avgpool first to reduce information lost
        if self.stride == 2:
            x_1x1_se = self.avgpool_s2_1(x_1x1_se)

        out_1x1 = self.conv1x1(x_1x1_se)
        out_1x1 = self.bn2(out_1x1)
        out_1x1_ratio = self.avgpool_add_1(out_1x1).squeeze(dim=2).squeeze(dim=2)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self,
        ...
        if not self.with_dcn or fallback_on_stride:
            self.conv2 = SelectFeatures_Conv_3_31_no_SF(planes, planes, stride=self.conv2_stride,ratio=0.5)
           # self.conv2 = build_conv_layer(
           #     conv_cfg,
           #     planes,
           #     planes,
